...
def get_cached_teachers(): return pd.read_sql("SELECT * FROM teachers", conn)
@st.cache_data(ttl=60)
def get_cached_halls(): return pd.read_sql("SELECT * FROM halls", conn)
...
def process_doc(doc_obj, row, h_name, h_city):
    ...
    repls = {'ZNAME': str(row.get('name', '')), 'ZID': str(row.get('id', '')), 'ZPHONE': phone_val, 'ZJOB': str(row.get('role', '')), 'ZHALL': str(h_name), 'ZLOC': str(h_city), 'ZWORK': str(row.get('school', '')), 'ZCITY': str(row.get('city', ''))}
...
# =====================================
# 4. الواجهة الرئيسية (التعديل المطلوب: المحاذاة)
# =====================================
...
with header_col2:
    st.markdown(f"""
        <div style="background-color: #1a1c23; padding: 10px 15px; border-radius: 8px; border-left: 5px solid #00ffcc; text-align: left; direction: ltr;">
    ...
    if q:
        ...
        for _, row in results.iterrows():
            with st.expander(f"👤 {row['name']} | القاعة: {row['hall'] or 'غير مكلف'}"):
                st.write(f"الهوية: {row['id']} | الجوال: {row['phone']}")
                c1, c2 = st.columns(2)
                with c1:
                ...
                if st.button("💾 حفظ التكليف", key=f"btn_save_{row['id']}"):
                    ...
                    conn.commit(); add_log("حفظ تكليف", f"تكليف {row['name']} في {sel_h}"); st.rerun()

# تبويب التوزيع التلقائي (بدون تغيير)
with tab_auto:
    st.markdown('<h2 class="move-to-right">🤖 التوزيع التلقائي</h2>', unsafe_allow_html=True)
    # ... الكود الخاص بالتوزيع التلقائي ...

# تبويب رفع البيانات (بدون تغيير)
with tab_upload:
    st.markdown('<h2 class="move-to-right">تحديث القالب والبيانات</h2>', unsafe_allow_html=True)
    # ... الكود الخاص بالرفع ...

# =====================================
# التعديل المطلوب: الإدارة والإحصائيات (المعداد)
# =====================================
with tab_manage:
    df_all_teachers = get_cached_teachers()
    st.markdown('<h3 class="move-to-right">📊 إحصائيات القاعات</h3>', unsafe_allow_html=True)
    assigned_halls = sorted(df_all_teachers[df_all_teachers['hall'].astype(str).str.len() > 0]['hall'].unique().tolist())
    
    if assigned_halls:
        h_choice = st.selectbox("اختر قاعة لعرض التفاصيل:", [""] + assigned_halls)
        if h_choice:
            df_hall_details = df_all_teachers[df_all_teachers['hall'] == h_choice]
            
            # المعداد (الطلب الثاني)
            c_s1, c_s2, c_s3, c_s4 = st.columns(4)
            c_s1.metric("رئيس قاعة", len(df_hall_details[df_hall_details['role'] == "رئيس قاعة"]))
            c_s2.metric("مساعد رئيس", len(df_hall_details[df_hall_details['role'] == "مساعد رئيس قاعة"]))
            c_s3.metric("مراقب", len(df_hall_details[df_hall_details['role'] == "مراقب"]))
            c_s4.metric("آذن", len(df_hall_details[df_hall_details['role'] == "آذن"]))
            
            st.dataframe(df_hall_details[['name', 'role', 'school', 'phone']], use_container_width=True)

# =====================================
# التعديل المطلوب: سجل العمليات (زر الحذف)
# =====================================
with tab_logs:
    st.markdown('<h2 class="move-to-right">📜 سجل العمليات</h2>', unsafe_allow_html=True)
    
    # إضافة زر حذف السجل (الطلب الأول)
    if st.button("🗑️ حذف كافة سجلات العمليات", key="del_all_logs"):
