...
st.markdown("""
    ...
    .counter-label { color: #bbb; font-size: 0.85rem; margin-bottom: 5px; }
    .counter-value { color: #00ffcc; font-size: 1.6rem; font-weight: bold; }
...
with header_col1:
    st.markdown(f'<div class="user-box"><span style="color: #bbb;">👤 الموظف الحالي:</span> <strong style="color: white;">{st.session_state.username}</strong></div>', unsafe_allow_html=True)
with header_col2:
    ...
    if q:
        ...
        for _, row in results.iterrows():
            ...
            with st.expander(f"👤 {row['name']} | القاعة: {row['hall'] or 'غير مكلف'}"):
                st.write(f"الهوية: {row['id']} | الجوال: {display_phone} | السكن: {row['city']}")
                # (بقية تفاصيل السيرش كما هي في كودك الأصلي...)
...
with tab_upload:
    ...
    # (كود رفع الملفات كما هو في ملفك...)

with tab_manage:
    df_all_teachers = get_cached_teachers()
    st.markdown('<h2 class="move-to-right">📊 إدارة القاعات والتقارير</h2>', unsafe_allow_html=True)
    ...
    if assigned_halls:
        ...
        if h_choice:
            ...
            with col_b1:
                # --- زر حذف السجل (تفريغ القاعة) ---
                if st.button(f"🗑️ تفريغ قاعة {h_choice}", key=f"del_h_{h_choice}"):
                    c.execute("UPDATE teachers SET hall='', role='', hall_city='', updated_by=? WHERE hall=?", (st.session_state.username, h_choice))
                    conn.commit()
                    add_log("تفريغ قاعة", f"تم مسح تكليفات قاعة {h_choice}")
                    st.success("تم تفريغ القاعة")
                    time.sleep(0.5)
                    st.rerun()
            
            with col_b2:
                if st.button(f"📄 إنشاء كتب {h_choice}"):
                    bulk_f = generate_bulk_word(df_hall_details, h_choice)
                    if bulk_f: st.download_button("📥 تحميل الوورد", bulk_f, f"تكليفات_{h_choice}.docx")
            
            with col_b3:
                output_h = io.BytesIO()
                df_hall_details.to_excel(output_h, index=False)
                st.download_button(f"📊 كشف إكسل {h_choice}", output_h.getvalue(), f"كشف_{h_choice}.xlsx")
