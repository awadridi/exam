...
def _clear_inquiry_state():
    """دالة مساعدة لإغلاق شاشة الاستعلام"""
...
def switch_system(mode):
    ...
    _clear_inquiry_state()  # ✅ إغلاق الاستعلام عند تغيير النظام
    st.cache_data.clear()
    st.rerun()

def switch_to_other_assignments():
    st.session_state['system_mode'] = "other_assignments"
    _clear_inquiry_state()  # ✅ إغلاق الاستعلام عند الانتقال لتكليفات أخرى
    st.cache_data.clear()
    st.rerun()
...
c.execute('''CREATE TABLE IF NOT EXISTS teachers 
             ...
             relative TEXT, relative_exam TEXT, gender TEXT)''')  # ✅ إضافة عمود الجنس

# 🔧 التعديل 2: إضافة عمود subject و gender
for col in ['relative', 'relative_exam', 'subject', 'gender']:
    try:
...
c.execute('''CREATE TABLE IF NOT EXISTS halls (hall_name TEXT PRIMARY KEY, city TEXT, capacity INTEGER)''')  # ✅ إضافة سعة القاعة
...
# ==================== تبويب التوزيع التلقائي ====================
    ...
    with tab_auto:
        ...
        if target_h2:
            # ✅ جلب بيانات محدثة مباشرة من قاعدة البيانات
            df_all = pd.read_sql("SELECT * FROM teachers", conn)
            
            # ✅ دالة تنظيف اسم القاعة
            def clean_hall(x):
                if pd.isna(x) or str(x).strip().lower() in ['', 'nan', 'none', 'null']: return ''
                return str(x).strip()
                
            df_all['hall_clean'] = df_all['hall'].apply(clean_hall)
            target_clean = clean_hall(target_h2)
            
            # ✅ حساب الأدوار الحالية
            ...
            # ✅ عرض حالة القاعة فوراً عند الاختيار (بدون حفظ)
            st.markdown(f"**📍 حالة القاعة المختارة: `{target_h2}`**")
            status_cols = st.columns(3)
            with status_cols[0]:
                if presidents_count > 0: st.warning("⚠️ لها رئيس قاعة بالفعل")
                else: st.success("✅ لا يوجد رئيس قاعة")
            with status_cols[1]:
                if assistants_count > 0: st.info(f"📋 مساعدين معينين: {assistants_count}/2")
                else: st.success("✅ لا يوجد مساعدين")
            with status_cols[2]:
                if janitors_count > 0: st.warning("⚠️ لها آذن بالفعل")
                else: st.success("✅ لا يوجد آذن")
                
            st.divider()
            
            # ✅ أعمدة الاختيار والتعيين
            ...
            # ✅ عرض المعينين حالياً فوراً (رئيس + مساعد + آذن فقط) مع خيار الإزالة
            st.markdown("---")
            st.markdown("### 👥 المعينون حالياً في هذه القاعة (رئيس، مساعد، آذن فقط)")
            assigned_in_hall = df_all[
                (df_all['hall_clean'] == target_clean) & 
                (df_all['role'].isin(['رئيس قاعة', 'مساعد رئيس قاعة', 'آذن']))
            ]
    ...
    with tab_inquiry:
        ...
        if st.session_state.run_query:
            ...
            if df.empty:
                ...
                if st.session_state.q_city != "الكل": mask &= (df['city'] == st.session_state.q_city)
...
if st.session_state.get('system_mode') == "tasheeh":
    ...
    st.markdown("""
        ...
        * { direction: rtl !important; }
        body, .stApp, main, section, div[class*="st-"] { direction: rtl !important; text-align: right !important; }
        h1, h2, h3, h4, h5, h6, p, span, label { direction: rtl !important; text-align: right !important; }
        input[type="text"], input[type="number"], input[type="date"], textarea, select, [data-baseweb="input"], [data-baseweb="select"] { direction: rtl !important; text-align: right !important; }
        button, [role="button"] { direction: rtl !important; }
        table, thead, tbody, tr, th, td { direction: rtl !important; text-align: right !important; }
        [data-testid="stTabs"], [data-testid="stTab"], .stTabs { direction: rtl !important; }
        .stAlert, .stInfo, .stSuccess, .stWarning, .stError, [data-testid="stAlert"], [data-testid="stMarkdown"] { direction: rtl !important; text-align: right !important; }
        [data-testid="stSelectbox"], [data-testid="stMultiselect"] { direction: rtl !important; }
        </style>
    """, unsafe_allow_html=True)
                ...
                if pool.empty:
                    ...
                    for _, t in pool.iterrows():
                        assignments.append({
                            ...
                            'hall_name': selected_hall, 'hall_city': hall_city, 'exam_name': selected_subj,
                            'exam_date': date_str, 'exam_day': day_name, 'school': t.get('school',''), 
                            'city': t.get('city',''), 'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M")
                        })
...
if st.session_state.get('system_mode') == "other_assignments":
    ...
    st.markdown("""
        ...
        [data-testid="stDataFrame"] table, [data-testid="stDataFrame"] th, [data-testid="stDataFrame"] td {
            direction: rtl !important; text-align: right !important; vertical-align: middle !important;
        }
        [data-testid="stDataFrame"] td { white-space: normal !important; word-wrap: break-word !important; max-width: 300px !important; }
        [data-testid="stDataFrame"] th { text-align: center !important; font-weight: bold !important; }
    ...
    def generate_other_letter(row):
        ...
        repls = {
            ...
            'ZSCHOOL': str(row.get('ZSCHOOL', '---')),
            'ZWORK': '' if is_guard else str(row.get('zwork', '---')),
    ...
    # ✅ دالة إنشاء ملف وورد واحد لجميع التكليفات
    ...
    st.markdown(f"""
        ...
        .custom-header-other {{ position: fixed; top: 0; left: 0; width: 100%; background-color: #1a1c23; color: white; text-align: center !important; padding: 15px 0; z-index: 999999; border-bottom: 2px solid #00ffcc; line-height: 1.5; direction: rtl; box-shadow: 0px 4px 10px rgba(0,0,0,0.5); }}
        .custom-header-other div {{ text-align: center !important; display: block; width: 100%; }}
        p, h3, h4, h5, h6, label, span, div {{ direction: rtl !important; text-align: right !important; }}
        input, textarea, button {{ direction: rtl !important; text-align: right !important; }}
        [data-testid="stTabs"] {{ direction: rtl !important; justify-content: flex-end !important; }}
        [data-testid="stTab"] {{ direction: rtl !important; }}
        [data-testid="stDataFrame"], table, thead, tbody, tr, td, th {{ direction: rtl !important; text-align: right !important; }}
        </style>
    ...
    with tab_guard:
        ...
        if 'guard_form_counter' not in st.session_state: st.session_state.guard_form_counter = 0
            ...
            with st.form("add_guard_form"):
                ...
                if submit_guard:
                    if g_zid and g_zname:
                        ...
                        st.session_state.guard_form_counter += 1
                        st.rerun()
                    else:
        ...
        with col2:
            ...
            if not df_guard.empty:
                ...
                arabic_cols = {'zid': 'رقم الهوية', 'zname': 'الاسم', 'zjob': 'المهمة', 'ZSCHOOL': 'الوظيفة الحالية', 'zwork': 'مكان التكليف', 'zloc': 'الموقع', 'zcity': 'مكان السكن', 'zdate': 'التاريخ', 'created_at': 'وقت الإنشاء'}
                df_display = df_display.rename(columns={k: v for k, v in arabic_cols.items() if k in df_display.columns})
                cols_to_show = ['الاسم', 'رقم الهوية', 'المهمة', 'الوظيفة الحالية', 'مكان التكليف', 'الموقع', 'مكان السكن', 'التاريخ']
                ...
                with col_btn1:
                    if st.button("📄 إنشاء ملف وورد واحد للجميع", type="primary", key="btn_word_guard"):
                        with st.spinner("جاري إنشاء الملف..."):
                            ...
                            if word_buffer:
                                st.download_button(label="📥 تحميل ملف الوورد", data=word_buffer.getvalue(), file_name=f"تكليفات_الحرس_{datetime.now().strftime('%Y%m%d')}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", key="dl_guard_word")
                ...
                delete_choice = st.selectbox("🗑️ اختر السجل للحذف:", [""] + [f"ID:{r['ID']} | {r['الاسم']} | {r['المهمة']}" for _, r in df_guard_display.iterrows()], key="guard_delete_select")
    ...
    with tab_parcels:
        ...
        if 'parcels_form_counter' not in st.session_state: st.session_state.parcels_form_counter = 0
        ...
        with col2:
            ...
            if not df_parcels.empty:
                ...
                with col_btn1:
                    if st.button("📄 إنشاء ملف وورد واحد للجميع", type="primary", key="btn_word_parcels"):
                        with st.spinner("جاري إنشاء الملف..."):
                            ...
                            if word_buffer:
                                st.download_button(label="📥 تحميل ملف الوورد", data=word_buffer.getvalue(), file_name=f"تكليفات_الطرود_{datetime.now().strftime('%Y%m%d')}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", key="dl_parcels_word")
                ...
                delete_choice = st.selectbox("🗑️ اختر السجل للحذف:", [""] + [f"ID:{r['ID']} | {r['الاسم']} | {r['المهمة']}" for _, r in df_parcels_display.iterrows()], key="parcels_delete_select")
    ...
    with tab_device:
        ...
        if 'device_form_counter' not in st.session_state: st.session_state.device_form_counter = 0
        ...
        with col2:
            ...
            if not df_device.empty:
                ...
                with col_btn1:
                    if st.button("📄 إنشاء ملف وورد واحد للجميع", type="primary", key="btn_word_device"):
                        with st.spinner("جاري إنشاء الملف..."):
                            ...
                            if word_buffer:
                                st.download_button(label="📥 تحميل ملف الوورد", data=word_buffer.getvalue(), file_name=f"تكليفات_جهاز_الامتحان_{datetime.now().strftime('%Y%m%d')}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", key="dl_device_word")
                ...
                delete_choice = st.selectbox("🗑️ اختر السجل للحذف:", [""] + [f"ID:{r['ID']} | {r['الاسم']} | {r['المهمة']}" for _, r in df_device_display.iterrows()], key="device_delete_select")
    ...
    with tab_committee:
        ...
        if 'committee_form_counter' not in st.session_state: st.session_state.committee_form_counter = 0
        ...
        with col2:
            ...
            if not df_committee.empty:
                ...
                with col_btn1:
                    if st.button("📄 إنشاء ملف وورد واحد للجميع", type="primary", key="btn_word_committee"):
                        with st.spinner("جاري إنشاء الملف..."):
                            ...
                            if word_buffer:
                                st.download_button(label="📥 تحميل ملف الوورد", data=word_buffer.getvalue(), file_name=f"تكليفات_لجنة_الامتحان_{datetime.now().strftime('%Y%m%d')}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", key="dl_committee_word")
                ...
                delete_choice = st.selectbox("🗑️ اختر السجل للحذف:", [""] + [f"ID:{r['ID']} | {r['الاسم']} | {r['المهمة']}" for _, r in df_committee_display.iterrows()], key="committee_delete_select")
