...
# قاعدة البيانات
db_path = os.path.join(os.getcwd(), "final_system_2026_fixed.db")
...
# =====================================
# 2. وظيفة الاستبدال العميقة (Deep Replace)
# =====================================
def generate_from_template(row):
    try:
        ...
        # دالة استبدال قوية جداً تتعامل مع النصوص المجزأة داخل Runs
        def replace_text_in_paragraph(paragraph, key, value):
            if key in paragraph.text:
                # محاولة الاستبدال البسيط أولاً
                for run in paragraph.runs:
                    if key in run.text:
                        run.text = run.text.replace(key, value)
                        return
                
                # إذا فشل (بسبب تجزئة التنسيق)، نقوم بدمج الـ runs وإعادة البحث
                full_text = "".join(run.text for run in paragraph.runs)
                if key in full_text:
                    new_text = full_text.replace(key, value)
                    # مسح النصوص القديمة ووضع النص الجديد في أول run للحفاظ على التنسيق
        ...
        # تنفيذ الاستبدال في كل فقرات المستند
        for p in doc.paragraphs:
            for key, val in replacements.items():
                replace_text_in_paragraph(p, key, val if val else "")
        
        # تنفيذ الاستبدال في الجداول
...
with tab_search:
    ...
    if q and not df_t.empty:
        ...
        for i, row in results.iterrows():
            # بناء العنوان ديناميكياً (الاسم أولاً ثم التفاصيل بعد الحفظ)
            title = f"👤 {row['name']}"
            ...
            with st.expander(title):
                c1, c2 = st.columns(2)
                with c1:
                    sel_hall = st.selectbox(f"القاعة {row['id']}", hall_list, 
                                          index=hall_list.index(row['hall']) if row['hall'] in hall_list else 0, key=f"h_{row['id']}")
                    sel_role = st.selectbox(f"الوظيفة {row['id']}", role_list, 
                ...
                with c2:
                    ...
                    if row['hall'] and row['role']:
                        ...
                        if file_data:
                            st.download_button(f"📥 تحميل التكليف", data=file_data, 
...
with tab_upload:
    ...
    with cu1:
        f_t = st.file_uploader("ملف المعلمين", type="xlsx")
        if f_t and st.button("رفع"):
